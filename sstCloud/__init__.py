...
class SstCloudClient:
                ...
                for house in self.houses_data:
                    ...
                    for device in response:
                        ...
                        for sensor, sensorType in _device_line.items():
                            if sensorType == "wired_sensor" and device["lines_enabled"][index] is True:
                ...
                for house in self.houseData():
                    for device in self.deviceData(house["id"]):
                        if device["is_connected"] is True:
                            ...
                            _is_changed = True
                if _is_changed is True:
                    self.dataChanged = True
                    self.lastRefresh = datetime.now()

    def _get_house_dev_counters(self, houseid, deviceid):
        url = "https://api.sst-cloud.com/houses/%s/devices/%s/counters" % (
            houseid,
            deviceid,
        )
    ...
    def _get_house_dev_wireless_sensors(self, houseid, deviceid):
        url = "https://api.sst-cloud.com/houses/%s/devices/%s/wireless_sensors" % (
            houseid,
            deviceid,
        )
        ...
        for sensor in sensors:
            ...
            del sensor["battery"]
            sensor["value"] = "off" if sensor["attention"] is False else "on"
            del sensor["attention"]

        return sensors

    def _get_house_dev_lines(self, houseid, deviceid):
        url = "https://api.sst-cloud.com/houses/%s/devices/%s/lines_status" % (
            houseid,
            deviceid,
        )
    ...
    def _get_house_statistics(self, houseid):
        ...
        url = "https://api.sst-cloud.com/statistic/houses/%s/?date_from=%s&date_to=%s&interval=%s" % (
            houseid,
            date_from,
            date_to,
            interval,
    ...
    def setValve(self, houseid, deviceid, value=False):
        url = "https://api.sst-cloud.com/houses/%s/devices/%s/valve_settings/" % (
            houseid,
            deviceid,
        )
        data = {"valve_settings": "opened" if value else "closed"}
    ...
    def setDryFlag(self, houseid, deviceid, value=False):
        url = "https://api.sst-cloud.com/houses/%s/devices/%s/dry_flag/" % (
            houseid,
            deviceid,
        )
        data = {"dry_flag": "on" if value else "off"}
        self._do_post_request(url, data)
        self._populate_full_data(True)
